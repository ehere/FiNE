                ...
                <table class="jobs-list">
                    ...
                    <c:forEach var="log" items="${requestScope.list}">
                        <tr>
                            ...
                            <td style="text-align: right;">
                                <a href="<%= F.asset("/profile")%>/${log.owner.id}" target="_blank">${log.owner.fullname}</a>
                            </td>
                            <td>
                                <a href="<%= F.asset("/profile")%>/${log.change_by.id}" target="_blank">${log.change_by.fullname}</a>
                            </td>
                            <td>
        ...
        <div class="pagination-wrapper ">
            <ul class="pagination pagination-lg">
                <c:if test="${requestScope.currentpage > 1}">
                    <li><a href="<%= F.asset("/admin/creditlog?page=")%>${requestScope.currentpage-1}">Previous</a></li>
                    </c:if>
                    <c:forEach begin="1" end="${requestScope.totalpage}" step="1" var="i">
                        <c:choose>
                            ...
                            <c:otherwise>
                            <li><a href="<%= F.asset("/admin/creditlog?page=")%>${i}">${i}</a></li>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:if test="${requestScope.currentpage < requestScope.totalpage}">
                        <c:if test="${requestScope.totalpage != 0}">
                        <li><a href="<%= F.asset("/admin/creditlog?page=")%>${requestScope.currentpage+1}">Next</a></li>
                        </c:if>
                    </c:if>
            </ul>
        </div>
